...
const Search = ({ updateBooks }) => {
  ...
  const searchBook = (evnt) => {
    if (evnt.key === 'Enter') {
      axios.get(`https://www.googleapis.com/books/v1/volumes?q=${searchTerm}&maxResults=20&key=AIzaSyDn_TMqEHDKMAlbPvTD2pqIv0vqnGq6V6A`)
        .then(res => {
  ...
  return (
    <SearchContainer>
      <SearchBar className="form">
        ...
        <button className="reset" type="reset" onClick={resetSearch}>
